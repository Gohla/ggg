...
fn main() {
  ...
  let mut job_graph_builder = job_queue.create_job_graph_builder();
  let mut prev = None;
  for i in 0i32..1024 {
    ...
    let curr = job_graph_builder.add_job(job);
    trace!("Creating job for {}, with index {:?}", i, curr);
    if let Some(prev) = prev {
      trace!("Adding dependency edge from {:?} to {:?} with value {}", prev, curr, i);
      job_graph_builder.add_dependency(prev, curr, i);
    }
    prev = Some(curr);
  }
  job_queue.set_job_graph(job_graph_builder).unwrap();
