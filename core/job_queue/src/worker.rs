...
use crate::{DependencyOutputs, DepKey, Handler, JobKey, Out};
...
impl<J: JobKey, D: DepKey, O: Out, H: Handler<J, D, O>> WorkerThread<J, D, O, H> {
  #[inline]
  pub(super) fn new(
    from_manager: Receiver<FromManager<J, D, O>>,
    to_manager: Sender<crate::manager::FromWorker<J, O>>,
  ...
  fn run(self, thread_index: usize) {
    ...
    loop {
      if let Ok((node_index, job_key, dependencies)) = self.from_manager.recv() {
        trace!("Running job {:?}", node_index);
        let output = (self.handler)(job_key, dependencies);
