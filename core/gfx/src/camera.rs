...
use common::screen::{PhysicalPosition, PhysicalSize};
...
impl CameraSys {
  pub fn new(viewport: PhysicalSize) -> CameraSys {
    CameraSys::with_speeds(viewport, 10.0, 0.05)
  }
  ...
  pub fn update(
    ...
    let pan_speed = self.pan_speed * frame_delta.as_s() as f32;
    ...
    let proj = {
      ...
      let min_z = 0.01f32;
      let max_z = 1000.0f32;
      projection::lh_yup::orthographic_wgpu_dx(min_x, max_x,
                                               min_y, max_y,
                                               min_z, max_z,
      )
    };
