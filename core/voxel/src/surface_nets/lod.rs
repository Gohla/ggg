...
type DeckDoubleArray<C> = <C as ChunkSize>::CellDeckDoubleArray<u16>;
...
type RowQuadArray<C> = <C as ChunkSize>::CellRowQuadArray<u16>;
...
impl<C: ChunkSize> SurfaceNetsLod<C> {
  ...
  pub fn extract_border_x(
    ...
    let mut cell_index_to_vertex_index = DeckDoubleArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_x(0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_x {
      Self::extract_global_positions_border_x(1, step, min_x, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
  ...
  pub fn extract_border_y(
    ...
    let mut cell_index_to_vertex_index = DeckDoubleArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_y(0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_y {
      Self::extract_global_positions_border_y(1, step, min_y, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
  ...
  pub fn extract_border_z(
    ...
    let mut cell_index_to_vertex_index = DeckDoubleArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_z(0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_z {
      Self::extract_global_positions_border_z(1, step, min_z, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
  ...
  pub fn extract_border_xy(
    ...
    let mut cell_index_to_vertex_index = RowQuadArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_xy(0, 0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_x {
      Self::extract_global_positions_border_xy(1, 0, step, min_x, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_y {
      Self::extract_global_positions_border_xy(0, 1, step, min_y, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
  ...
  pub fn extract_border_yz(
    ...
    let mut cell_index_to_vertex_index = RowQuadArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_yz(0, 0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_y {
      Self::extract_global_positions_border_yz(1, 0, step, min_y, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_z {
      Self::extract_global_positions_border_yz(0, 1, step, min_z, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_yz {
      Self::extract_global_positions_border_yz(1, 1, step, min_yz, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }

    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_yz {
      Self::extract_quads_border_yz(1, 1, chunk_sample_array, &cell_index_to_vertex_index, chunk_mesh);
    }
  }
  ...
  pub fn extract_border_xz(
    ...
    let mut cell_index_to_vertex_index = RowQuadArray::<C>::new(u16::MAX);
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples {
      Self::extract_global_positions_border_xz(0, 0, step, min, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_x {
      Self::extract_global_positions_border_xz(1, 0, step, min_x, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_z {
      Self::extract_global_positions_border_xz(0, 1, step, min_z, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }
    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_xz {
      Self::extract_global_positions_border_xz(1, 1, step, min_xz, chunk_sample_array, &mut cell_index_to_vertex_index, chunk_mesh);
    }

    if let ChunkSamples::Mixed(chunk_sample_array) = chunk_samples_xz {
      Self::extract_quads_border_xz(1, 1, chunk_sample_array, &cell_index_to_vertex_index, chunk_mesh);
    }
  }
  ...
  fn extract_global_positions_border_x(
    ...
    cell_index_to_vertex_index: &mut DeckDoubleArray<C>,
    chunk_mesh: &mut ChunkMesh,
  ) {
  ...
  fn extract_global_positions_border_xy(
    ...
    min: UVec3,
    chunk_sample_array: &ChunkSampleArray<C>,
    cell_index_to_vertex_index: &mut RowQuadArray<C>,
    chunk_mesh: &mut ChunkMesh,
  ) {
    for z in 0..C::CELLS_IN_CHUNK_ROW {
      let border_cell = BorderCell::new(x, y, z);
  ...
  fn extract_quads_border_x(
    x: u32,
    chunk_sample_array: &ChunkSampleArray<C>,
    cell_index_to_vertex_index: &DeckDoubleArray<C>,
    chunk_mesh: &mut ChunkMesh,
  ) {
    for z in 0..C::CELLS_IN_CHUNK_ROW {
      for y in 0..C::CELLS_IN_CHUNK_ROW {
  ...
  fn extract_quads_border_xy(
    ...
    y: u32,
    chunk_sample_array: &ChunkSampleArray<C>,
    cell_index_to_vertex_index: &RowQuadArray<C>,
    chunk_mesh: &mut ChunkMesh,
  ) {
    for z in 0..C::CELLS_IN_CHUNK_ROW {
      let border_cell = BorderCell::new(x, y, z);
      ...
      let vertex_index = cell_index_to_vertex_index.index(border_cell_index);
      if vertex_index == u16::MAX { continue; }
      Self::extract_quad_border_xy(border_cell, border_cell_index, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    }
  }
  ...
  fn extract_quads_border_yz(
    ...
    for x in 0..C::CELLS_IN_CHUNK_ROW {
      ...
      let vertex_index = cell_index_to_vertex_index.index(border_cell_index);
      if vertex_index == u16::MAX { continue; }
      Self::extract_quad_border_yz(border_cell, border_cell_index, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    }
  }
  ...
  fn extract_quads_border_xz(
    ...
    for y in 0..C::CELLS_IN_CHUNK_ROW {
      ...
      let vertex_index = cell_index_to_vertex_index.index(border_cell_index);
      if vertex_index == u16::MAX { continue; }
      Self::extract_quad_border_xz(border_cell, border_cell_index, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    }
  }
  ...
  fn extract_quad_border_x(
    ...
    Self::extract_quad_parallel_y::<ShapeX<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    Self::extract_quad_parallel_z::<ShapeX<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
  ...
  fn extract_quad_border_y(
    ...
    let value_a_negative = chunk_sample_array.sample_index(min_voxel_index).is_sign_negative();
    Self::extract_quad_parallel_x::<ShapeY<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    Self::extract_quad_parallel_z::<ShapeY<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
  ...
  fn extract_quad_border_z(
    ...
    let value_a_negative = chunk_sample_array.sample_index(min_voxel_index).is_sign_negative();
    Self::extract_quad_parallel_x::<ShapeZ<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
    Self::extract_quad_parallel_y::<ShapeZ<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
  ...
  fn extract_quad_border_xy(
    ...
    let min_voxel_index = border_cell.to_cell_border_xy::<C>().to_min_voxel_index::<C>();
    let value_a_negative = chunk_sample_array.sample_index(min_voxel_index).is_sign_negative();
    Self::extract_quad_parallel_z::<ShapeXY<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
  ...
  fn extract_quad_border_yz(
    ...
    let min_voxel_index = border_cell.to_cell_border_yz::<C>().to_min_voxel_index::<C>();
    let value_a_negative = chunk_sample_array.sample_index(min_voxel_index).is_sign_negative();
    Self::extract_quad_parallel_x::<ShapeYZ<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
  ...
  fn extract_quad_border_xz(
    ...
    let min_voxel_index = border_cell.to_cell_border_xz::<C>().to_min_voxel_index::<C>();
    let value_a_negative = chunk_sample_array.sample_index(min_voxel_index).is_sign_negative();
    Self::extract_quad_parallel_y::<ShapeXZ<C>, _>(border_cell, border_cell_index, min_voxel_index, value_a_negative, chunk_sample_array, cell_index_to_vertex_index, chunk_mesh);
  }
